...
function initCouchdbVars {
  if [ $# -ne 1 ]; then
     echo "Usage: initCouchdbVars <NUM>: $*"
     exit 1
  fi
  NUM=$1
  COUCHDB_NAME=couchdb${NUM}
  COUCHDB_PORT=$((5984+1000*(${NUM}-1)))
}
...
function initPeerVars {
   ...
   PEER_PORT1=$((7051+1000*(${NUM}-1)))
   PEER_PORT2=$((7053+1000*(${NUM}-1)))
