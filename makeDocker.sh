...
function writeOrderer {
   ...
   echo "  $ORDERER_NAME:
    ...
    volumes:
      - ./scripts:/scripts
      - ./$DATA:/$DATA
      - ../hyperledger/production/orderer/$ORDERER_NAME:/var/hyperledger/production/orderer
    networks:
      - $NETWORK
    depends_on:
      - setup
...
function writeCouchdb {
  echo "  $COUCHDB_NAME:
    ...
    volumes:
      - ../hyperledger/production/$COUCHDB_NAME:/opt/couchdb/data
    ports:
      - $COUCHDB_PORT:5984
    networks:
      - $NETWORK
...
function writePeer {
   ...
   echo "    working_dir: $MYHOME
    ...
    volumes:
      ...
      - ../hyperledger/production/$PEER_NAME:/var/hyperledger/production/
    ports:
      - $PEER_PORT1:7051
      - $PEER_PORT2:7053
    networks:
